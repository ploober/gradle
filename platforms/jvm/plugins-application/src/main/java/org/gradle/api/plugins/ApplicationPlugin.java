...
public abstract class ApplicationPlugin implements Plugin<Project> {
    ...
    private void addCreateScriptsTask(Project project, JvmFeatureInternal mainFeature, JavaApplication pluginExtension) {
        project.getTasks().register(TASK_START_SCRIPTS_NAME, CreateStartScripts.class, startScripts -> {
            ...
            startScripts.getClasspath().from(jarsOnlyRuntimeClasspath(mainFeature));
            startScripts.getMainModule().set(pluginExtension.getMainModule());
            startScripts.getMainClass().set(pluginExtension.getMainClass());
            startScripts.getApplicationName().set(project.provider(pluginExtension::getApplicationName));
            startScripts.getGitRef().set(project.provider(DefaultGradleVersion.current()::getGitRevision));
            startScripts.getOutputDir().set(project.getLayout().getBuildDirectory().dir("scripts"));
            startScripts.getExecutableDir().set(project.provider(pluginExtension::getExecutableDir));
            startScripts.getDefaultJvmOpts().set(project.provider(pluginExtension::getApplicationDefaultJvmArgs));
            JavaPluginExtension javaPluginExtension = project.getExtensions().getByType(JavaPluginExtension.class);
